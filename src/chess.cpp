...
void Board::toggleTurn() {
	turn =
		(turn == PlayerColor::black) ? PlayerColor::white : PlayerColor::black;
}
...
void Board::transformPiece(const Square &square, const PieceName &pieceName) {
	...
	case PieceName::knight:
		cells[square.col][square.row] = std::make_unique<Knight>(
			square.col, square.row, turn,
			std::format("{}/img/{}/knight1.png", PROJECT_ROOT_DIR,
						playerColorCStr(turn)));
		break;
...
void Board::drawOffer() const {
	...
	const std::string str =
		std::format("{} offered a draw!", playerColorCStr(turn));
...
const Square Board::findMove(const Move &move) {
	...
	const Column colStart =
		(move.from.col == Column::none) ? Column::A : move.from.col;
	const Column colEnd =
		(move.from.col == Column::none) ? Column::H : move.from.col;
	...
	case PieceName::pawn:
		for (Column col = colStart; col <= colEnd; ++col) {
			for (Row row = rowStart; row <= rowEnd; ++row) {
				...
				if (piece != nullptr && piece->getPlayerColor() == turn &&
					piece->getName() == PieceName::pawn) {
	...
	case PieceName::knight:
		for (Column col = colStart; col <= colEnd; ++col) {
			for (Row row = rowStart; row <= rowEnd; ++row) {
				auto &piece = cells[col][row];
				if (piece != nullptr && piece->getPlayerColor() == turn &&
					piece->getName() == PieceName::knight) {
	...
	case PieceName::bishop:
		for (Column col = colStart; col <= colEnd; ++col) {
			for (Row row = rowStart; row <= rowEnd; ++row) {
				auto &piece = cells[col][row];
				if (piece != nullptr && piece->getPlayerColor() == turn &&
					piece->getName() == PieceName::bishop) {
...
void Board::getPawnMoves(const std::unique_ptr<Piece> &piece,
	...
	if (col != Column::A) {
		...
		if (capturePiece != nullptr &&
			capturePiece->getPlayerColor() != color) {
		...
		for (const int8_t &rowDiff : {(int8_t)-1, (int8_t)1}) {
			if (col + colDiff >= Column::A && col + colDiff <= Column::H &&
				...
				if (cells[col + colDiff][row + rowDiff] == nullptr ||
					cells[col + colDiff][row + rowDiff]->getPlayerColor() !=
						color) {
...
const bool Board::moveIsCheck(const Square &from, const Square &to) {
	...
	if (cells[to.col][to.row] != nullptr)
		tempPiece = std::move(cells[to.col][to.row]);
	...
	for (Column col = Column::A; col <= Column::H; ++col) {
		for (Row row = Row::_1; row <= Row::_8; ++row) {
			auto &piece = cells[col][row];
			if (piece != nullptr && piece->getName() == PieceName::king &&
				piece->getPlayerColor() != turn) {
				king = Square{col, row};
			}
		}
	}
